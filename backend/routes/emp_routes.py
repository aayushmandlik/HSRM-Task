...
async def create_employee(data: EmployeeCreate, current_admin: TokenPayload = Depends(require_admin)):
    # Verify user exists in users_collection
    user = await users_collection.find_one({"_id": ObjectId(data.user_id),"email":data.email})
    if not user:
        raise HTTPException(status_code=400, detail="User does not exist or invalid id/email")
...
async def update_employee(user_id: str, data: EmployeeUpdate, current_admin: TokenPayload = Depends(require_admin)):
...
async def delete_employee(user_id: str, current_admin: TokenPayload = Depends(require_admin)):
